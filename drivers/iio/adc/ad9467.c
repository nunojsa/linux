...
#define AD9652_DEF_OUTPUT_MODE		0x00
...
#define AD9649_DEF_OUTPUT_MODE		0x00
					 ...
					 bool invert)
{
	...
	if (st->info->has_dco && st->info->has_dco_invert) {
		...
		for (c = 0; c < st->info->num_channels; c++) {
			 ...
			 */
			 dev_dbg(dev, "Invalid point(%u, inv:%u) for CH:%u\n",
				 point, invert, c);
			__set_bit(point + invert * test_points, st->calib_map);
			break;
		}
	}
