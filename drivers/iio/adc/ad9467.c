					  ...
					  loff_t *ppos)
{
	...
	if (chan->mode == AN877_ADC_TESTMODE_PN9_SEQ ||
		...
		len = snprintf(buf, sizeof(buf), "Running %s Test:\n\t",
		...
		len = snprintf(buf, sizeof(buf), "No test Running...\n");
	} else {
		len = snprintf(buf, sizeof(buf), "Running %s Test on CH:%u\n",
...
static void ad9467_debugfs_init(struct iio_dev *indio_dev)
{
	...
	for (chan = 0; chan < st->info->num_channels; chan++) {
		sprintf(attr_name, "in_voltage%u_test_mode", chan);
		st->chan_test[chan].idx = chan;
		st->chan_test[chan].st = st;
		debugfs_create_file(attr_name, 0600, d, st->chan_test,
